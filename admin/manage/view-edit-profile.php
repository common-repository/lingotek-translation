...
foreach ( $settings as $key => $setting ) {
	?>
	<tr id="<?php echo esc_attr( $key . '_row' ); ?>">
		<th scope="row"><?php printf( '<label for="%s">%s</label>', esc_attr( $key ), esc_attr( $setting['label'] ) ); ?></th>
		<td>
		<?php
		printf( '<select name="%1$s" id="%1$s" %2$s>', esc_attr( $key ), in_array( $key, array( 'upload', 'download' ) ) ? esc_attr( $disabled ) : '' );
		...
		?>
			...
			if ( 'project_id' == $key ) {
				...
				<label for="update_callback"><?php esc_html_e( 'Update the callback url for this project.', 'lingotek-translation' ); ?></label>
																								   <?php
			}
...
<h3><?php esc_html_e( 'Target languages', 'lingotek-translation' ); ?></h3>
		...
		foreach ( $settings as $key => $setting ) {
			...
			?>
					<tr>
						...
						<td>
						<?php
						printf( '<select name="%1$s" id="%1$s" %2$s>', esc_attr( $custom_key ), in_array( $key, array( 'upload', 'download' ) ) ? esc_attr( $disabled ) : '' );
						foreach ( $setting['options'] as $option_id => $option_title ) {
	...
	<td>
		<?php
			$select_all_on_change = "
									...
									";
		?>
		<input type="checkbox" name="target_select_all" id="target_select_all" onchange="<?php echo esc_attr( esc_js( $select_all_on_change ) ); ?>">
