...
class Lingotek_Post_Actions extends Lingotek_Actions {
	...
	public function manage_promotions() {
		$ltk_promotion = sanitize_text_field( filter_input( INPUT_GET, 'ltk-promotion' ) );
		if ( null !== $ltk_promotion && 'dismiss' === $ltk_promotion ) {
			update_option( 'lingotek_professional_promotion_shown', true, false );
		} elseif ( null !== $ltk_promotion && 'view' === $ltk_promotion ) {
			update_option( 'lingotek_professional_promotion_shown', true, false );
	...
	public function manage_actions() {
		...
		switch ( $action ) {
			...
			case 'lingotek-upload':
				...
				$id       = absint( filter_input( INPUT_GET, 'post' ) );
			...
			case 'lingotek-copy':
				...
				$post_to_copy = get_post( (int) absint( filter_input( INPUT_GET, 'post' ) ) );
	...
	public function lingotek_save_meta_boxes() {
		...
		global $post;
		$profile_choice   = sanitize_text_field( filter_input( INPUT_POST, 'lingotek_profile_meta' ) );
