...
abstract class Lingotek_Actions {
	...
	public static function display_error_icon( $name, $api_error, $additional = '' ) {
		return sprintf(
			...
			esc_attr( self::$icons[ $name ]['icon'] ),
			esc_attr( self::$icons[ $name ]['title'] )  . "\n" . esc_html( $api_error ),
			esc_html( $additional )
		);
	}
	...
	protected function _add_bulk_actions( $bulk_actions ) {
		foreach ( self::$actions as $action => $action_data ) {
			...
			if ( !$action_data['per_locale'] ) {
                ...
                $bulk_actions[ 'bulk-lingotek-' . $action ] = sprintf(
                /* translators: %s: Action data */
                    esc_html__( 'Bulk Lingotek %s', 'lingotek-translation' ),
			...
			if ( isset( $action_data['per_locale'] ) && true === $action_data['per_locale'] ) {
				foreach ( PLL()->model->get_languages_list() as $language ) {
                    $bulk_actions[ 'bulk-lingotek-' . $action . ':' . $language->locale ] = sprintf(
                        __( '%1$s for %2$s', 'lingotek-translation' ),
                        $action_data['action'],
		...
		foreach ( array_keys( self::$actions ) as $action ) {
			if ( null !== filter_input( INPUT_GET, "bulk-lingotek-$action" ) ) {
				...
				if ( null !== filter_input( INPUT_GET, 'target_locale' ) ) {
					$data['target_locale'] = sanitize_lingotek_locale( filter_input( INPUT_GET, 'target_locale' ) );
				}
				if ( null !== filter_input( INPUT_GET, 'locales' ) ) {
					$data['locales'] = explode( ',', filter_input( INPUT_GET, 'locales' ) );
					array_map( 'sanitize_lingotek_locale', explode( ',', filter_input( INPUT_GET, 'locales' ) ) );
				}
	...
	protected function _manage_actions( $action ) {
		if ( null !== filter_input( INPUT_GET, 'document_id' ) ) {
			$document_id = sanitize_key(filter_input( INPUT_GET, 'document_id' ));
		...
		switch ( $action ) {
			...
			case 'lingotek-request':
        		...
        		$sanitized_locale = null !== $locale ? sanitize_lingotek_locale( $locale ) : null;
				...
				null !== $sanitized_locale ? $document->request_translation( $sanitized_locale ) : $document->request_translations();
				break;
			...
			case 'lingotek-download':
				...
				null !== $sanitized_locale ? $document->create_translation( $sanitized_locale) : $document->create_translations();
				break;
			
			case 'lock-target':
				check_admin_referer( 'lock-target' );
				$locale = sanitize_lingotek_locale( filter_input( INPUT_GET, 'locale') );
			...
			case 'lingotek-delete':
				...
				*/
				$document->delete();
				if ( null !== filter_input( INPUT_GET, 'lingotek_redirect' ) && filter_var(filter_input(INPUT_GET, 'lingotek_redirect'), FILTER_VALIDATE_BOOLEAN) === true ) {
			...
			case 'lingotek-delete-translation':
				...
				$target_id     = absint(filter_input(INPUT_GET, 'target_id'));
			...
			case 'lingotek-cancel-translation':
				...
				$target_locale = sanitize_lingotek_locale( filter_input( INPUT_GET, 'target_locale' ) );
				$document_id   = sanitize_key( filter_input( INPUT_GET, 'document_id' ) );
	...
	public function ajax_download() {
		...
		$document = $this->lgtm->get_group( $this->type, sanitize_key( filter_input( INPUT_POST, 'id' ) ) );
		if ( $document ) {
	...
	public function ajax_request() {
		...
		$document = $this->lgtm->get_group( $this->type, sanitize_text_field( filter_input( INPUT_POST, 'id' ) ) );
		if ( $document ) {
	...
	public function ajax_delete() {
		...
		$id       = sanitize_key( filter_input( INPUT_POST, 'id' ) );
	...
	public function ajax_cancel_translation() {
		...
		$id            = sanitize_key( filter_input( INPUT_POST, 'id' ) );
		$target_locale = sanitize_lingotek_locale( filter_input( INPUT_POST, 'target_locale' ) );
		$document      = $this->lgtm->get_group( $this->type, $id );
		// phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
	...
	public function ajax_estimate_cost() {
		...
		$document_id   = sanitize_key( filter_input( INPUT_GET, 'document_id' ) );
		$locale        = sanitize_lingotek_locale( filter_input( INPUT_GET, 'locale' ) );
		$lingotek_auth = sanitize_text_field( filter_input( INPUT_GET, 'Authorization-Lingotek' ) );
