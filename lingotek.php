...
define( 'LINGOTEK_BRIDGE_URL', 'https://marketplace.lingotek.com' );
define( 'LINGOTEK_NEW_ACCOUNT_LANDING_PAGE', 'https://insight.strakertranslations.com/lingotek-freemium-upgrade-connectors' );
...
class Lingotek {
	...
	public function __construct() {
		...
		$plugin = sanitize_text_field( filter_input( INPUT_GET, 'plugin' ) );
		...
		$action = isset( $action ) ? $action : sanitize_text_field( filter_input( INPUT_POST, 'action' ) );
	...
	public function lingotek_new_ui() {
		$content = __( 'Ray Enterprise new UI is ready! If you still need the Polylang based columns, customize those in "Screen Options"', 'lingotek-translation' );
		...
		$args = array(
			...
			'width'    => 380,
			'title'    => __( 'Ray Enterprise New UI', 'lingotek-translation' ),
			'content'  => $content,
			'buttons'  => $buttons,
		);
	...
	public function lingotek_plugin_migration() {
		...
		if ( $updates_have_run ) {
			...
			add_action(
                ...
                <p>' . esc_html__( 'Upgrades for the Ray Enterprise translation module have completed.', 'lingotek-translation' ) . '</p>
              </div>';
				}
			);
		}
	...
	protected function do_for_all_blogs( $what ) {
		// network.
		$network_wide = absint( filter_input( INPUT_GET, 'networkwide' ) );
	...
	public static function get_profiles() {
		...
		foreach ( $profiles as $k => $v ) {
			if ( in_array( $k, array( 'automatic', 'manual', 'disabled' ), true ) ) {
                $profile_names = array(
                    'automatic' => esc_html__( 'Automatic', 'lingotek-translation' ),
                    'manual'    => esc_html__( 'Manual', 'lingotek-translation' ),
                    'disabled'  => esc_html__( 'Disabled', 'lingotek-translation' ),
                );

                // Localize canned profile names.
                $profiles[ $k ]['name'] = $profile_names[ $k ];
			}
		}
	...
	public function pll_inactive_notice() {
		...
		printf(
			...
			esc_html( __( 'Ray Enterprise Translation requires Polylang to work. ', 'lingotek-translation' ) ),
			esc_url( $url ),
	...
	public function pll_old_notice() {
		...
		printf(
			...
			sprintf(
				esc_html( __( 'Ray Enterprise Translation requires Polylang %s to work. Please upgrade Polylang.', 'lingotek-translation' ) ),
	...
	public function lingotek_activation_pointer() {
		$content = __( 'You’ve just installed Ray Enterprise Translation! Click below to activate your account and automatically translate your website for free!', 'lingotek-translation' );
	...
	public function lingotek_professional_translation_pointer() {
		$content = __( 'Ray Enterprise Professional Translation is now available!', 'lingotek-translation' );
